...
class NoiseScheduleVP:
    ...
    def marginal_log_mean_coeff(self, t):
        ...
        if 'discrete' in self.schedule:
            ...
            s = 0.008
            log_alpha = torch.log(torch.cos((t + s) / (1 + s) * math.pi / 2))
            log_alpha = torch.clamp(log_alpha, min=-5.1)  # ✅ FIX 1
    ...
    def marginal_std(self, t):
        ...
        exp_2log_mean = torch.exp(2. * log_mean)
        exp_2log_mean = torch.clamp(exp_2log_mean, max=1. - 1e-5)  
        return torch.sqrt(1. - exp_2log_mean)


    def marginal_prob(self, t):
        log_mean_coeff = self.marginal_log_mean_coeff(t)
        alpha_t = torch.exp(log_mean_coeff)
        sigma_t_sq = 1. - torch.exp(2. * log_mean_coeff)
        sigma_t_sq = torch.clamp(sigma_t_sq, min=1e-5)  # ✅ FIX 3
    ...
    def get_noiseLevel(self, t):
        ...
        return torch.log((alpha_t ** 2) / (sigma_t ** 2 + 1e-5))

    def add_noise(self, data, t):
    # """
    # Add noise to the input data at time step t.
    
    # Args:
    #     data (torch.Tensor): The clean input data x_0 (shape: [B, ...])
    #     t (torch.Tensor): A tensor of time steps (shape: [B]) with values in [0, T]
    
    # Returns:
    #     x_t (torch.Tensor): The noisy version of the input at time t
    #     noise (torch.Tensor): The sampled noise epsilon
    # """
    # Sample Gaussian noise with the same shape as input data
        noise = torch.randn_like(data)

    # Compute alpha_t and sigma_t for each t
    # Ensure shape broadcasting: [B, 1, 1, ...] if needed
    ...
    # Apply the forward diffusion formula: x_t = alpha_t * x_0 + sigma_t * epsilon
